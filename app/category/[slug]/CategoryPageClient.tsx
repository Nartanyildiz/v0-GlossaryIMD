      ...
      <main className="min-h-screen bg-black text-[#00D0B4] p-6 md:p-12">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <Link href="/">
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight hover:text-[#00D0B4]/80 transition-colors">
                GLOSSAIRE IMD
              </h1>
            </Link>
            <MenuOverlay />
          ...
          <div className="text-center py-8">
            <p className="text-[#00D0B4]">{t("noResults")}</p>
          </div>
        </div>
      </main>
    )
    ...
    <main className="min-h-screen bg-black text-[#00D0B4] p-6 md:p-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative mb-8">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#00D0B4]/50 h-5 w-5" />
          <Input
            type="text"
            placeholder={t("searchInCategory", { category: translatedCategory })}
            className="pl-10 bg-[#1E2A3B] border-none text-[#00D0B4] placeholder:text-[#00D0B4]/50 h-12"
            value={searchTerm}
            ...
            filteredTerms.map((term, index) => (
              <div key={index} className="bg-[#1E2A3B] p-6 transition-all hover:bg-[#263447]">
                <div className="flex justify-between items-start mb-2">
                  <h2 className="text-xl font-medium text-[#00D0B4]">{term.word}</h2>
                  <span className="text-xs px-2 py-1 bg-black/30 rounded-full text-[#00D0B4]">
                    {translatedCategory}
                  </span>
                </div>
                <p className="text-[#00D0B4]/90">{term.definition[language]}</p>
              </div>
            ))
          ) : (
            <div className="text-center py-8">
